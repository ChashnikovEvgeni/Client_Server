...
namespace SharpClient
{
	...
	class Message
	{
		...
		static T fromBytes<T>(byte[] buff)
		{
			T data = default(T);
			int size = Marshal.SizeOf(data);
			IntPtr i = Marshal.AllocHGlobal(size);
			Marshal.Copy(buff, 0, i, size);
			data = (T)Marshal.PtrToStructure(i, data.GetType());
		...
		void send(Socket s)
		{
			...
			if (header.size != 0)
			{
				s.Send(cp866.GetBytes(data), header.size, SocketFlags.None);
			}
		}
		...
		MessageTypes receive(Socket s)
		{
			...
			if (header.size > 0)
			{
				...
				data = cp866.GetString(b, 0, header.size);
